...
function handleOrientation(event){
    if(event.beta!==null && event.gamma!==null){
        inputMode='gyro';
        // 调整陀螺仪的幅度，使得旋转幅度更大
        gyroTargetX=Math.max(-20, Math.min(20, event.beta / 3));  // 增加幅度
        gyroTargetY=Math.max(-20, Math.min(20, event.gamma / 3)); // 增加幅度
    }
}
...
function renderLoop(){
    ...
    if(inputMode==='gyro'){
        ...
        const dx = mouseX - cx;
        const dy = mouseY - cy;
        targetX = (-dy / (rect.height / 2)) * 12;  // 增加幅度
        targetY = (dx / (rect.width / 2)) * 12;  // 增加幅度
    }
...
cardFlip.addEventListener('click', ()=>{
    ...
    currentTiltX = 0; currentTiltY = 0;  // 重置 tilt 防止跳动
});
    ...
    for(let i = 0; i < 40; i++){
        ...
        (function(p, targetX, targetY){
            ...
            function animate(timestamp){
                ...
                if(progress < 1){
                    let cx = targetX * progress;
                    let cy = targetY * progress;
...
function handleEasterEgg(){
    ...
    if(value === 'sherman'){
        ...
        launchFireworks();
        const fireworksTimer = setInterval(launchFireworks, 1200);
        setTimeout(()=>{
