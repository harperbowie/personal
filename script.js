...
// 检测 iOS Safari
var isIosSafari = /iP(hone|od|ad)/.test(navigator.userAgent) && /Safari/.test(navigator.userAgent) && !/Chrome|CriOS|Firefox/.test(navigator.userAgent);

if(isIosSafari){
    // Safari: 用户交互触发
    window.addEventListener('touchstart', enableGyroSafari, {once:true});
    window.addEventListener('click', enableGyroSafari, {once:true});
} else {
    // 非 Safari: 直接注册
    window.addEventListener('deviceorientation', handleOrientation, true);
